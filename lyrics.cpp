...
void Lyrics::update_tag_maps() {
    ...
    for (int i = 0; i < lines.length(); i++) {
        ...
        QRegularExpressionMatchIterator tag_match_iter;
        tag_match_iter = tag_re.globalMatch(lines[i]);
        QStringList tags;
        ...
        for (int i = 0; i < tags.length(); i++) {
            ...
            if (valid_time) _time_map.insert(time, content);
...
QString Lyrics::order_tags() {
    ...
    foreach (const QTime &time, _time_map.keys()) {
        ...
        lrc_ordered += QString("%1%2\n").arg(tag, _time_map.value(time));
    }
    return lrc_ordered;
}
